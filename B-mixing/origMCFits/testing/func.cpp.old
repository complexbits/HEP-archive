		      ...
		      Double_t dm, Double_t s, Double_t t)
{
  ...
  if (tag == 0) ll = 1.;
  if (tag == -1) ll = 0.5*uNmix(t, t0, dm, s,tag);
  if (tag == 1) ll = 0.5*mix(t, t0, dm, s,tag);
  
  return ll;
}

Double_t uNmix(Double_t t, Double_t t0, Double_t dm, Double_t s,Int_t tag)
{
...
Double_t mix(Double_t t, Double_t t0, Double_t dm, Double_t s,Int_t tag)
{
  Double_t aa = (t/(TMath::Sqrt(2.)*s)) - (s/(TMath::Sqrt(2.)*t0));  
  Double_t bb = (s*s/(2.*t0*t0)) - t/t0;
  Double_t cc = s*dm/(TMath::Sqrt(2.));
...
Double_t lftmosc_plt(Double_t *x, Double_t *par)
{
  ...
  if (tag == -1) func = nEvts*(max-min)/nBins*0.5*uNmix(t, t0, dm, s,tag);
  if (tag == 1) func = nEvts*(max-min)/nBins*0.5*mix(t, t0, dm, s,tag);
