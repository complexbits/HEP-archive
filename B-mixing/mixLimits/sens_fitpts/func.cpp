		      ...
		      Double_t dm, Double_t s, Double_t t)
{
  ...
  if (tag == 0) ll = 1.;
  if (tag == -1) ll = mixing(t, t0, dm, s, tag);
  if (tag == 1) ll = mixing(t, t0, dm, s, tag);
...
Double_t lftmosc_plt(Double_t *x, Double_t *par)
{
  ...
  if (tag == -1) func = nEvts*(max-min)/nBins* mixing(t, t0, dm, s, tag);
  if (tag == 1) func = nEvts*(max-min)/nBins* mixing(t, t0, dm, s, tag);

  return func;

}
Double_t lftmosc_pltu(Double_t *x, Double_t *par)
{
  Double_t t   = x[0];
  Double_t t0  = par[0];
  Double_t dm  = par[1];
...
Double_t lftmosc_plt_d(Double_t *x, Double_t *par)
{
  ...
  if (tag == -1) func = nEvts*(max-min)/nBins* ((1.-mistag)*mixing(t, t0, dm, s, -1.) +
			       mistag*mixing(t, t0, dm, s, 1.));
  if (tag == 1) func = nEvts*(max-min)/nBins* ((1.-mistag)*mixing(t, t0, dm, s, 1.) +
			      mistag*mixing(t, t0, dm, s, -1.));
...
Double_t lftmosc_plt_du(Double_t *x, Double_t *par)
{
  Double_t t       = x[0];
  Double_t t0      = par[0];
  Double_t dm      = par[1];
  ...
  if (tag == -1) func = ((1.-mistag)*mixing(t, t0, dm, s, -1.) +
			       mistag*mixing(t, t0, dm, s, 1.));
  if (tag == 1) func = ((1.-mistag)*mixing(t, t0, dm, s, 1.) +
			      mistag*mixing(t, t0, dm, s, -1.));
