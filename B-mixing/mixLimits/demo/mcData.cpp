...
void mixmasta_mc()
{
  ...
  for (Int_t i=0; i<nEvts; i++){
	...
	Double_t tSmear = gRandom->Gaus(0., tSigma);
      ...
      tsm[i]=tSmear;
      lt_sm1[i]=rTime[i]+tSmear;
      // Smearing to account for (unmeasured) neutrinos in the decay
      if (nSigma == 0.){
	Double_t nSmear = 0.;
      }
      else{
	Double_t nSmear = gRandom->Gaus(0.,(nSigma*rTime[i]));
      }
      nsm[i]=nSmear;
      Double_t mTime = rTime[i] + tSmear + nSmear;
