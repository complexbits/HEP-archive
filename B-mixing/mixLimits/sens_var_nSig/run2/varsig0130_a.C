...
// vary this between 0.1 and 0.4 in the varsig0130*.C's
...
void varsig0130_a()
{
  ...
  TGraph *SigFit = new TGraph(20);
  SigFit->SetName("SigFit");
  SigFit->SetTitle("Sigma in Fit vs. dm");
  
  for(Int_t i=0.;i<20.;i++){


    oscpar1_init = 0.5+(0.5*i);
    ...
    // Calculate (weighted) avg. nSigma for fitting

    Int_t bweight = 0.;
    Double_t nSigavg = 0.;
    for(Int_t jb=0.; jb<nBins; jb++){

      bweight = h2->GetBinContent(jb+1);
      nSigavg = nSigavg + bweight*(nSigma*(min+(jb*(max-min)/nBins)));
      
    }
    nSigavg = nSigavg/nEvts;
    oscpar2_init = tSigma + nSigavg;    
    cout << "nSigavg = "<< nSigavg << ", oscpar2_init = " << oscpar2_init << endl;   
  
    SigFit->SetPoint(i,oscpar1_init,oscpar2_init);
  ...
  TCanvas *vardm = new TCanvas("vardm","varsig0130_a",800,400);
  vardm->Divide(2,1);
  ...
  TFile var_dm("varsig0130_a.root", "recreate");
  a1.Write();
  var_dm.Close();
