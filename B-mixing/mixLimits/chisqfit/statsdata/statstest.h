...
// Thu Apr 21 19:44:07 2005 by ROOT version 4.03/02
// from TTree dstagTree/AA Ds
// found on file: bs_mixing_MC.root
//////////////////////////////////////////////////////////
...
#ifdef statstest_cxx
statstest::statstest(TTree *tree)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("bs_mixing_MC.root");
      if (!f) {
         f = new TFile("bs_mixing_MC.root");
      }
...
Int_t statstest::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t statstest::LoadTree(Long64_t entry)
{
...
void statstest::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("lbmctrue",&lbmctrue);
   fChain->SetBranchAddress("pvposmc",pvposmc);
   fChain->SetBranchAddress("svposmc",svposmc);
   fChain->SetBranchAddress("pvpos",pvpos);
   fChain->SetBranchAddress("svpos",svpos);
   ...
   fChain->SetBranchAddress("qtagin",&qtagin);
   fChain->SetBranchAddress("osci",&osci);
   fChain->SetBranchAddress("oscimc",&oscimc);
   Notify();
}

Bool_t statstest::Notify()
{
   ...
   b_lbmctrue = fChain->GetBranch("lbmctrue");
   b_pvposmc = fChain->GetBranch("pvposmc");
   b_svposmc = fChain->GetBranch("svposmc");
   b_pvpos = fChain->GetBranch("pvpos");
   b_svpos = fChain->GetBranch("svpos");
   ...
   b_qtagin = fChain->GetBranch("qtagin");
   b_osci = fChain->GetBranch("osci");
   b_oscimc = fChain->GetBranch("oscimc");

   return kTRUE;
}

void statstest::Show(Long64_t entry)
{
...
Int_t statstest::Cut(Long64_t entry)
{
