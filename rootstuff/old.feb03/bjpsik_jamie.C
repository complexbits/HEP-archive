...
//		  cout << jentry << "  B-id: " << iLoop << " Count-B-id: " << good_B[iLoop] << "  Psi-id-this-B: " << psi_id << "  Count-Psi-id: " <<  num_Bs_with_psi[psi_id] << endl << endl;
//		  cout << "Event number: " << jentry << "   B-id " << iLoop << "  Psi-id: " << psi_id << " B-id-[3]-count: " << good_B[3] << endl << endl;
      ...
      for (Int_t jLoop=0; jLoop < 50;  jLoop++){		 // Loop through all of the possible psi_id's we may have found
      
	  if (num_Bs_with_psi[jLoop]==1){			// Continue if and only if the particular psi_id was logged exactly once.
		
	      for (Int_t iLoop=0; iLoop < 40; iLoop++){		// Loop through *all* the possible B's we may have found
	      
	        if (good_B[iLoop]>0){				// Take only the B's which passed all the cuts
		  
		  if (jLoop == BPSIK_id[iLoop]){		// Continue if the psi_id for a given good-B is the same as the one which 
								// had exactly one occurrence.
...
//		      cout << "         B-id " << iLoop << " passed all cuts with good_B[" << iLoop << "] = " << good_B[iLoop] << endl << endl;
					
		      winner_bmass -> Fill(BPSIK_mass[iLoop]);	    // Fill histograms with the masses of the B-Psi combo
		      winner_psimass -> Fill(PSI_mass[jLoop]);	    // that occurred exactly once.
