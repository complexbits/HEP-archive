...
TFile finished_data("histograms-filled.root","recreate");
  ...
  TH1S *bpsik_pt_pt = new TH1S("bpsik_pt_pt", "B pt with more cuts", 100, 0., 25.);
  ...
  TH1S *delta_phi = new TH1S("delta_phi", "Phi difference: B - Jet", 100, 0, 7);
  ...
  cout << "-----------------------------------------------------------------------------------------------" << endl;
  cout << "Event jet# njet   dphi      jphi     track_phi    bpsik_pt   jet_pt" << endl;
  cout << "-----------------------------------------------------------------------------------------------" << endl;
	     ...
	     if (BPSIK_mass[iLoop] > 0.){
	        ...
	        if (PSI_cft1[psi_id] >= 10 && PSI_cft2[psi_id] >= 10 &&
		      ...
		      PSI_dl[psi_id] > .1){
			    ...
			    bpsik_pt_pt -> Fill(BPSIK_pt[iLoop]);
			    ...
			    if (BPSIK_mass[iLoop] > 5.1 && BPSIK_mass[iLoop] < 5.4){
			    	...
			    	Float_t jphi = BPSIK_phi[iLoop];
				
				for (Int_t kLoop=0; kLoop < TRACKJET_njet; kLoop++){
				
					Float_t dphi = fabs(jphi - TRACKJET_phi[kLoop]);
					delta_phi -> Fill(dphi);
					
					if (dphi < 0.2){
					
			//		  cout << "-----------------------------------------------------------------------------------------------" << endl;
  			//		  cout << "Event       dphi       jphi	    track_phi	  bpsik_pt     jet_pt   jet#  njet" << endl;
  			//		  cout << "-----------------------------------------------------------------------------------------------" << endl;
					  cout << jentry << "  " << kLoop << "   " << TRACKJET_njet << "     " << dphi << "   " << jphi << "   " << TRACKJET_phi[kLoop]<< "   " << BPSIK_pt[iLoop] << "   " << TRACKJET_pt[kLoop] <<  endl;
						
//						cout << "Evt #:" << jentry << "  dphi= " << dphi << "  jphi= " << jphi << "  track_phi= " << TRACKJET_phi[kLoop] << endl;
//						cout << "                              bpsik_pt= " << BPSIK_pt[iLoop] << "          jetpt= " << TRACKJET_pt[kLoop] << endl;
//						cout << "                              jet# (kLoop)= " << kLoop << "            njet= " << TRACKJET_njet << endl;
//						cout << "                              bpsik# (iLoop)= " << iLoop << "          psi_id= " << psi_id << endl << endl; 
					}
