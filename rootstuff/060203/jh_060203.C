...
TFile plotz("jh_060203.root","recreate");
  ...
  // Kpt Histograms with B-mass cuts
  TH1F* kpt_lo_bmc = new TH1F("kpt_lo_bmc", "0.5<Kpt<1.0",50,0.5,1.0);
  TH1F* kpt_hi_bmc = new TH1F("kpt_hi_bmc", "Kpt>1.0",50,1.0,1.5);
  ...
  //   for (Int_t jentry=0; jentry<30000;jentry++) {
   for (Int_t jentry=0; jentry<nentries;jentry++) {
	...
	//---------- KPT's for 5.15GeV<Bmass<5.35GeV-----------------

	if (Kch2<=10 && chi2<10 && dl_sig>=4.5 && Ksmt>1 && coll>0.95 && massc1<5.35 && massc1>5.15){
	  kpt_lo_bmc->Fill(Kpt);
	  kpt_hi_bmc->Fill(Kpt);
	}
